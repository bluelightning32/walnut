...
TEST(MLogNEstimator, ErrorUpTo1024) {
  ...
  for (size_t i = 1; i < 1024; ++i) {
    EXPECT_NEAR(static_cast<double>(estimator.Estimate(i, i)),
                i * std::log2(i),
...
TEST(MLogNEstimator, HalfM) {
  MLogNEstimator estimator;
  for (size_t i = 1; i < 1024; ++i) {
    EXPECT_NEAR(static_cast<double>(estimator.Estimate(i/2, i)),
                (i/2) * std::log2(i),
...
TEST(MLogNEstimator, QuarterM) {
  MLogNEstimator estimator;
  for (size_t i = 1; i < 1024; ++i) {
    EXPECT_NEAR(static_cast<double>(estimator.Estimate(i/4, i)),
                (i/4) * std::log2(i),
...
TEST(MLogNEstimator, QuarterN) {
  ...
  for (size_t i = 4; i < 1024; ++i) {
    EXPECT_NEAR(static_cast<double>(estimator.Estimate(i, i/4)),
                i * std::log2(i/4),
...
void TestSplit(size_t base_polys, size_t neg_extra, size_t pos_extra) {
  for (size_t i = 0; i < base_polys; ++i) {
    ...
    EXPECT_NEAR(static_cast<double>(GetSplitCost(neg_total,
                                                 /*neg_exclude=*/neg_extra,
                                                 pos_total,
