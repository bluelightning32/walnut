...
TEST(PlanarRange, InputTooShort) {
  ...
  using InputIterator = std::vector<Vertex3<32>>::const_iterator;
  ...
  EXPECT_EQ(std::vector<Vertex3<32>>(result.begin(), result.end()),
            input);
...
TEST(PolygonGetNextPlanar, AllPlanar) {
  std::vector<Vertex3<32>> input{Vertex3<32>(0, 0, 0), Vertex3<32>(1, 0, 0),
                                 Vertex3<32>(1, 1, 0)};
  using InputIterator = std::vector<Vertex3<32>>::const_iterator;
  InputIterator remaining_begin;
  InputIterator remaining_end;
...
TEST(PlanarRange, StopsAtNonplanar) {
  ...
  EXPECT_EQ(result.size(), 3);
  EXPECT_TRUE(result.plane().normal().IsSameDir(Vector3<>(0, 0, 1)));
  EXPECT_THAT(std::vector<Vertex3<32>>(result.begin(), result.end()),
