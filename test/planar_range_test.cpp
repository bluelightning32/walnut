...
TEST(PlanarRange, InputTooShort) {
  ...
  using InputIterator = std::vector<Point3>::const_iterator;
  ...
  EXPECT_EQ(std::vector<Point3>(result.begin(), result.end()),
            input);
...
TEST(PolygonGetNextPlanar, AllPlanar) {
  std::vector<Point3> input{Point3(0, 0, 0), Point3(1, 0, 0),
                                 Point3(1, 1, 0)};
  using InputIterator = std::vector<Point3>::const_iterator;
  InputIterator remaining_begin;
  InputIterator remaining_end;
...
TEST(PlanarRange, StopsAtNonplanar) {
  std::vector<Point3> input{Point3(1, 0, 0),
                                 Point3(0, 1, 0),
                                 Point3(0, -1, 1),
                                 Point3(0, 0, 0)};
  using InputIterator = std::vector<Point3>::const_iterator;
  ...
  EXPECT_EQ(result.size(), 3);
  EXPECT_TRUE(result.plane().normal().IsSameDir(Vector3(0, 0, 1)));
  EXPECT_THAT(std::vector<Point3>(result.begin(), result.end()),
